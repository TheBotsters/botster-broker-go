...
// DELETE /secrets/api/{id}/grant/{agentId}
func (s *Server) handleWebSecretsRevoke(w http.ResponseWriter, r *http.Request) {
	accountID := r.Header.Get("X-Account-ID")
	if accountID == "" {
		jsonError(w, 401, "Not authenticated")
		return
	...
	s.DB.LogAudit(&accountID, &agentID, nil, "secret.revoke", secret.Name)
	jsonResponse(w, 200, map[string]bool{"ok": true})
}
